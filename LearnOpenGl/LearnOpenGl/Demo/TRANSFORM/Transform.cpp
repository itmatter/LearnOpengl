...
int runMyTransform() {
    ...
    MyProgram myProgram = MyProgram(transformVertexShaderStr, transformFragmentShaderSrc);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(transformVertices), transformVertices, GL_STATIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(transformVerticesIndices), transformVerticesIndices, GL_STATIC_DRAW);

    //解绑VAO
    glBindVertexArray(0);
    //计算索引个数
    squareIndicesCount = sizeof(transformVerticesIndices)/sizeof(transformVerticesIndices[0]);
